...
while i <= no_devices:
    if (bandwidth[i-1]>=service_bandwidth):
        netw_cost = device_conn[0][i]
        netw_cost = netw_cost + + device_conn[i][no_devices+1]
        if (netw_cost < lowest_cost):
            lowest_cost = netw_cost;
            selected_server = i;

    i += 1

print("Edge server with least network cost of service deployment: %d" % selected_server)
